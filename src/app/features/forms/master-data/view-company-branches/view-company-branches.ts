...
export class ViewCompanyBranches {
  ...
  updatePaginatedList() {
    if (this.isAnyFilterActive) {
      this.totalItems = this.filteredCompanyList.length;
      const start = (this.currentPage - 1) * this.itemsPerPage;
      this.paginatedCompanyList = this.filteredCompanyList.slice(
        start,
        start + this.itemsPerPage,
      );
    } else {
      this.totalItems = this.totalPagesCount * this.itemsPerPage;
      this.paginatedCompanyList = this.companyList;
    }
  }
