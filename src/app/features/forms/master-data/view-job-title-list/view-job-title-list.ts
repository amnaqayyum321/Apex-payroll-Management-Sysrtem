...
export class ViewJobTitleList {
  ...
  const page = this.isAnyFilterActive ? 0 : backendPage;
    this.FormSv.GetJobTitle(page, pageSize).subscribe({
      next: (response: any) => {
        this.loader.hide();
        this.JobTitleList = response.data.sort(
        (a: any, b: any) =>
      ...
      error: () => {
        this.loader.hide();
        this.toastr.error('Error fetching job titles list');
      },
    });
  }
  ...
  applyFilter() {
    ...
    this.filteredJobTitleList = this.JobTitleList.filter((item) => {
      const matchesSearch =
  ...
  updatePaginatedList() {
    if (this.isAnyFilterActive) {
      this.totalItems = this.filteredJobTitleList.length;
      const start = (this.currentPage - 1) * this.itemsPerPage;
      this.paginatedJobTitleList = this.filteredJobTitleList.slice(
        start,
